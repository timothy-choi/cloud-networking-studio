...
from app.services.runtime_strategy_registry import require_runtime_strategy
...
_DEFAULT_SUBNET = "10.250.0.0/24"
_DEFAULT_SUBNET_COMPOSE = "${CNS_RUNTIME_SUBNET:-10.250.0.0/24}"
...
def _compose_export_subnet() -> str:
    """Default subnet for local compose IP allocation (override via CNS_RUNTIME_SUBNET in .env)."""
    return _DEFAULT_SUBNET


def _remap_intent_ip_to_subnet(ip_raw: str, subnet_cidr: str) -> str | None:
    """Map a topology intent IP into the export subnet when prefixes differ."""
    ...
    if ip in network:
        return ip_raw
    if ip.version != 4 or network.version != 4 or network.prefixlen != 24:
        return None
    host_octet = int(str(ip).split(".")[-1])
        ...
        if ip_raw:
            try:
                ...
                if ip not in network:
                    remapped = _remap_intent_ip_to_subnet(ip_raw, subnet_cidr)
                    if remapped:
                        ip_raw = remapped
                        ip = ipaddress.ip_address(ip_raw)
                    else:
...
def generate_docker_compose(
    ...
    *,
    ...
    subnet = subnet_cidr or _compose_export_subnet()
    ...
    lines = [
        ...
        "# Network subnet is configurable via CNS_RUNTIME_SUBNET in .env (see .env.example).",
        "services:",
    ]
    ...
    lines.extend(
        [
            ...
            "    ipam:",
            "      config:",
            f"        - subnet: {_DEFAULT_SUBNET_COMPOSE}",
            "",
        ]
    )
    return "\n".join(lines)
    ...
    if planning_only:
        ...
        lines.extend(
            [
                ...
                "   ```",
                "",
                "2. If Docker reports an overlapping address space error, edit `.env` and set "
                "`CNS_RUNTIME_SUBNET` to a non-conflicting CIDR (for example `10.251.0.0/24`). "
                "Static `ipv4_address` values in `docker-compose.yml` are assigned for the default "
                f"subnet (`{_DEFAULT_SUBNET}`); update them if you change the subnet.",
                "",
