...
1. Waits for **`GET /`** and **`GET /api/health`** on **`http://<EIP>.sslip.io`** (production deploy uses **HTTP-only** Caddy until HTTPS on sslip is polished; **`CNS_BASE_URL`** must be **`stack_base_url_sslip_http`**, no **`-L`**). Tunable **`CNS_CURL_*`** and inner retries help slow **sslip.io** DNS.
