...
| **Production stack smoke** | Builds and starts [`docker-compose.prod.yml`](../docker-compose.prod.yml) on the runner with **`AUTH_REQUIRE_LOGIN=true`**, waits with [`scripts/wait_caddy_edge.sh`](../scripts/wait_caddy_edge.sh) / [`scripts/ci_wait_for_stack.sh`](../scripts/ci_wait_for_stack.sh) until **`/`** and **`/api/health`** succeed through Caddy, then runs [`scripts/prod_smoke_test.sh`](../scripts/prod_smoke_test.sh). |

The smoke job proves that **images build**, **Compose wiring works**, **Postgres + API + static UI + Caddy** start together, and **Step 34** auth-aware API paths work: **unauthenticated** topology create returns **401**, and **JWT + project** flows allow **topology create/list** (same as the dashboard).
...
- Live checks: browser bundle at `/`, API health at `/api/health`, **register/login** smoke user, **project** context, **topology create** (draft) and **list** with **`Authorization: Bearer`**.
...
When `CNS_HEAVY_SMOKE=1` is set (enabled in CI on `ubuntu-latest`), [`scripts/prod_smoke_test.sh`](../scripts/prod_smoke_test.sh) also builds a tiny two-node topology, **deploys** against the runner’s Docker engine (backend mounts `/var/run/docker.sock`), waits for deployment success, then **destroys**. All topology/deploy calls use the same **Bearer** token from the smoke register/login step. This depends on Docker-in-CI behavior and image pulls; it is slower and can fail on registry or engine quirks.

Health-only mode: set **`AUTH_SMOKE=0`** to run edge wait + **`/`** / **`/api/health`** only (no JWT or topology checks).
