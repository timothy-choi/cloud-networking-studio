...
See **`docs/EPHEMERAL_CI_ENVIRONMENTS.md`** for ephemeral-specific behavior (PR stacks use **HTTP** `stack_base_url_sslip_http` for smoke, **Caddyfile.prod** via **`CADDYFILE_SSLIP`** so there is no **308** to HTTPS, and **HTTPS** on sslip remains the production and Vercel path).
