...
**GitHub Actions (`deploy-production.yml`):** before SSH deploy, the workflow checks repository secrets **`POSTGRES_PASSWORD`** and **`CNS_CORS_ORIGINS`**. On the instance, the SSH step **writes** **`~/cloud-networking-studio/.env`** from secrets and Terraform outputs (no secret values in logs). Production includes **`SSLIP_HOST`**, **`CADDYFILE_SSLIP=./deploy/Caddyfile.sslip`**, **`CNS_CADDY_SITE_ADDRESS=<EIP>.sslip.io`**, **`CNS_CADDY_AUTO_HTTPS=on`**, **`DATABASE_URL`**, and related **`CNS_*`** keys so Caddy serves **HTTPS** on sslip (Let's Encrypt) and **HTTP :80**. **`set +x`** is used while secrets are expanded; only **`.env` key names** are printed after write.
...
4. Runs **Terraform** under **`infra/terraform`**: **`backend.ci.hcl`**, **`terraform init`**, **`fmt -check`**, **`validate`**, **`plan`**, **`apply`**, **`terraform output`** (including sslip URLs for smoke, CORS, and Vercel defaults).
...
| EC2 (compose on instance) | **`POSTGRES_PASSWORD`**, **`CNS_CORS_ORIGINS`** | Required: **`.env`** on each deploy (see **EC2 bootstrap**). Include Vercel origins and **`https://<EIP>.sslip.io`** (and **`http://…`** if you still use HTTP). |
