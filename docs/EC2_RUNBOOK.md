...
Deploy **Cloud Networking Studio** on one EC2 host using [docker-compose.prod.yml](../docker-compose.prod.yml) (bundled **Postgres**, API, static UI, Caddy). For **AWS RDS** instead of storing application data on the Compose volume, set **`DATABASE_URL`** to RDS — see [RDS.md](RDS.md). For environment variables and architecture, see [DEPLOYMENT.md](DEPLOYMENT.md).
...
| Backend unhealthy | `docker compose -f docker-compose.prod.yml logs backend postgres` — wrong `DATABASE_URL` or Postgres not ready |
