...
| **EC2 `.env`** | **HTTPS** on **`api.cloudnetstudio.com`** (**`docker-compose.caddy-https.yml`**, **`Caddyfile.public-https`**, ACME volumes) + secrets | **HTTP-only** Caddy on sslip (**`Caddyfile.prod`**, **`CNS_CADDY_AUTO_HTTPS=off`**, **`CNS_CADDY_SITE_ADDRESS=:80`**) + secrets |
...
3. **SSH:** fetch **`scripts/ec2_bootstrap_docker.sh`**, wait for **cloud-init**, install/wait for **Docker** (retries, up to 5 minutes), then clone/checkout, **`.env`** (heredoc), **`docker compose … up`** (logs only on **config** / **up** failure).
