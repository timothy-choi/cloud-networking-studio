  ...
  return (
    <div className="rounded-xl border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900/80">
      <div className="border-b border-zinc-200 px-4 py-3 dark:border-zinc-800">
        ...
        <p className="text-xs text-zinc-500">Stop / restart / kill · auto-refreshed</p>
      </div>
      <div className="max-h-56 overflow-auto">
        {sorted.length === 0 ? (
          <p className="p-4 text-sm text-zinc-500">No injections recorded.</p>
        ) : (
            ...
            <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
              {sorted.map((f) => (
                <tr key={f.id} className="font-mono text-[11px] text-zinc-700 dark:text-zinc-300">
                  <td className="whitespace-nowrap px-3 py-2 text-zinc-500">{formatTs(f.created_at)}</td>
